...
if num_outputs == 1:
    f = np.concatenate([f1], axis=1)[..., None]
elif num_outputs == 2:
...
noise = np.linalg.cholesky(noise_scale)[None] @ np.random.multivariate_normal(np.zeros(num_outputs), np.eye(num_outputs), f.shape[0])[..., None]
y = f + noise
...
len_f_w = 60.0
# weights = np.array([[1., 0.1], [0.5, 0.5], [0.1, 1.]])
# weights = weights[:num_outputs, :num_latents]
...
plt.legend()
plt.title('GP Regression Network')
plt.xlabel('$X$')
plt.show()

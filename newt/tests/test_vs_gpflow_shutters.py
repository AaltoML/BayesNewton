...
def initialise_newt_model(var_f, len_f, var_y, x, y):
    r = np.unique(x[:, 1])
    print(r)
    kernel = newt.kernels.SpatioTemporalMatern52(variance=var_f, lengthscale_time=len_f, lengthscale_space=len_f, z=r)
...
def test_gradient_step(var_f, len_f, var_y):
    ...
    gv = objax.GradValues(inf, newt_model.vars())

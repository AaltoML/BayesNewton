...
if model_type == 0:
    ...
    model = newt.models.MarkovVariationalGPMeanField(kernel=kern, likelihood=lik, X=t, R=r, Y=Y, parallel=parallel)
elif model_type == 2:
    model = newt.models.InfiniteHorizonVariationalGP(kernel=kern, likelihood=lik, X=t, R=r, Y=Y, parallel=parallel)
...
unscented_transform = newt.cubature.Unscented(dim=1)  # 5th-order unscented transform

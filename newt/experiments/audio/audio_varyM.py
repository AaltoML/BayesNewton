...
kern = newt.kernels.Independent([sub1, sub2, sub3, mod1, mod2, mod3])
...
if method == 0:
    ...
    inf = newt.inference.ExpectationPropagation(power=0.9, cubature=Unscented())  # power=1 unstable in sparse case
elif method == 3:
    inf = newt.inference.ExpectationPropagation(power=0.5, cubature=Unscented())
elif method == 4:
    inf = newt.inference.ExpectationPropagation(power=0.01, cubature=Unscented())
elif method == 5:
    inf = newt.inference.VariationalInference(cubature=Unscented())
...
predict_mean_modulators = newt.utils.softplus(predict_mean[:, 3:])
...
if plot_final:
    ...
    posterior_mean_modulators = newt.utils.softplus(posterior_mean[:, 3:])
    posterior_mean_sig = np.sum(posterior_mean_subbands * posterior_mean_modulators, axis=-1)
    posterior_var_subbands = posterior_var[:, :3]
    posterior_var_modulators = newt.utils.softplus(posterior_var[:, 3:])

...
time_points = np.array([200, 1000, 2000, 3000, 4000, 5000])
spatial_points = np.array([10, 50, 100, 150, 200, 250, 300, 350, 400, 450, 500])

if len(sys.argv) > 1:
    model_type = int(sys.argv[1])
    nt_ind = int(sys.argv[2])
    nr_ind = int(sys.argv[3])
    # fold = int(sys.argv[3])
...
nt = time_points[nt_ind]  # temporal grid points (x-axis)
scale = 1000 / nt
...
# inf_args = {"cubature": bayesnewton.cubature.Unscented()}
...
with open("output/" + str(model_type) + "_" + str(nr_ind) + "_" + str(nt_ind) + "_" + str(int(parallel))
          + "_" + cpugpu + ".txt", "wb") as fp:
